...
app.get('/api/notices/stream', async (req, res) => {
  ...
  try {
      ...
      },
      {
        ...
        action: async (results) => {
          ...
          if (results && results.length > 0) {
            await vectorDB.addMemories(results.map(item => ({
              title: item.title,
              summary: item.summary || item.title,
              url: item.link
            })));
          }
          return { saved: results?.length || 0 };
        }
      },
      {
        name: '完成',
    ...
    // 执行流式爬虫
    await sse.streamScraperProgress(steps);
